...
class TBLog:
    ...
    def update_imgs(self, tb_img_dict, it):
        """
        Args
            tb_img_dict: contains images for updating tensorboard
            it: contains information of iteration (int).
        """
        for key, value in tb_img_dict.items():
            self.writer.add_image(tag=key, img_tensor=value, global_step=it)        
...
def simple_accuracy(input, target):
    size = target.numel() if torch.is_tensor(target) else target.size
    return 100 * float(np.count_nonzero((input == target).cpu())) / size

    
def ce_loss(logits, targets, use_hard_labels=True, reduction='none', weights=None):
    """
    ...
    if use_hard_labels:
        if weights == None:
            ...
            return F.cross_entropy(logits, targets, reduction=reduction, weight=weights)
    else:
